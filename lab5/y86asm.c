...
{
    symbol_t *symiter=symtab->next;
    while(symiter){
        if (!strncmp(symiter->name, name, strlen(name)))
            return symiter;
        symiter=symiter->next;
    }
    return NULL;
}/*}}}*/
...
parse_t parse_symbol(char **ptr, char **name)/*{{{*/
{
    ...
    /* skip the blank and check */
    SKIP_BLANK(cur);
    if (IS_END(cur) || !IS_LETTER(cur))
        return PARSE_ERR;
    /* allocate name and copy to it */
    while(IS_LETTER(cur+len)||IS_DIGIT(cur+len)){
        len++;
    }
    tmp=(char *)malloc(len*sizeof(char));
    strncpy(tmp,cur,len);//'\0'??
    cur+=len;
    /* set 'ptr' and 'name' */
    *name = tmp;
    *ptr = cur;
...
parse_t parse_imm(char **ptr, char **name, long *value)/*{{{*/
{
    ...
    if(IS_IMM(cur)){
        cur++;
        rt_sign=parse_digit(&cur,value);
    }
    /* if IS_LETTER, then parse the symbol */
    else if(IS_LETTER(cur))
        rt_sign=parse_symbol(&cur,name);
    else
        return PARSE_ERR;
    if(rt_sign==PARSE_ERR)
        return PARSE_ERR;
...
/*
 * parse_line: parse a line of y86 code (e.g., 'Loop: mrmovl (%ecx), %esi')BEL
...
int relocate(void)
{
    ...
    while (rtmp) {
        ...
        if(stmp == NULL){//_print_err_
            err_print("Unknown symbol:'%s'", rtmp->name);
            return -1;
        }
        /* relocate y86bin according itype */
        long * p=(long *)&(rtmp->y86bin->codes[rtmp->y86bin->bytes-4]);
        *p=stmp->addr;
        /* next */
        rtmp = rtmp->next;
    }
...
int binfile(FILE *out)
{
    ...
    line_t *wrtiter = y86bin_listhead->next;     
    int front_addr=0;
    /* binary write y86 code to output file (NOTE: see fwrite()) */
    while(wrtiter){
        if(wrtiter->y86bin.bytes){
            int FILL_ZERO=wrtiter->y86bin.addr-front_addr;
            while(FILL_ZERO-->0){
                fprintf(out,"%c",0);
            }
            fwrite(wrtiter->y86bin.codes,sizeof(byte_t),wrtiter->y86bin.bytes,out);
            front_addr=wrtiter->y86bin.addr+wrtiter->y86bin.bytes;
        }
        wrtiter=wrtiter->next;
    }
    return 0;
}
